...
class SimpleSensorAngleChecker:
    ...
    def manual_angle_measurement(self, image_path):
        ...
        yaw_angle = float(180 - self._calculate_angle(yaw_points[0], yaw_points[1]))
        print(f"Yaw角: {yaw_angle:.2f}°")
        
        # 3. 选择关键点计算Pitch角（垂直方向）
        print("\n步骤3: 选择Pitch角测量点")
        ...
        # 4. 选择关键点计算Roll角（旋转角度）
        print("\n步骤4: 选择Roll角测量点")
        print("点击选择传感器上的两个特征点（如边缘）")
        roll_points = self._select_two_points(img, "Select Roll Measurement Points")
        if roll_points is None:
            return None
        
        roll_angle = float(45 - self._calculate_angle(roll_points[0], roll_points[1]))
    ...
    def _save_result(self, result_img):
        ...
        filename = f"sensor_angle_check_{timestamp}.jpg"
...
def main():
    # 创建检测器，设置公差为1度
    checker = SimpleSensorAngleChecker(tolerance=1.0)
    
    # 设置参考角度（理想安装角度）
    # 假设理想安装：Yaw=0°, Pitch=90°, Roll=0°
