...
try:
    ...
    import Get_vehicles_car_config
...
class host_env():
    ...
    def __output__(self):
        ...
        self.output_for_dirve_car = "当前车辆 lidar Pose异常，请重启后再试"
        self.IPC_j6_logs_pose = "lidar spin exceed max delay to OMC clock or latest pose"
        self.get_orin_logs = f"grep -r 'failed' /log/qcraft sort -u |head -n 10"
        self.log_list = f"cat {self.j6_collect_log_path} |strings |grep -E '{self.IPC_j6_logs_pose}'"
    ...
    # def Get_vehicles_car_config(self):
    #     need_car_ids = self.get_env_hosts()
    ...
    def config_lidar_status(self):
        lidar_list_result = Get_vehicles_car_config.command_shell_script(shell_path="./GET_COF.sh")
        
        result_list = []
        
        if lidar_list_result[0] == 0:
            lidar_str = lidar_list_result[1]    
            
            # 使用正则表达式提取所有 (ip, name) 对
            pattern = r'\(\s*"([^"]+)"\s*,\s*"([^"]+)"\s*\)'
            matches = re.findall(pattern, lidar_str)
            
            print(f"找到 {len(matches)} 个雷达设备")
            
            for ip, name in matches:
                print(f"正在检查: {name} ({ip})")
    ...
    # def clock_lidar_status(self):
    #     try:
    #         lidar_result = self.status_lidars()
    #         for ip , status_code in lidar_result:
    #             if status_code == 0:
    #                 clock_command = f"{self.clock_cmd} {ip}"
    ...
    def get_pose_status(self):
        # print("init")
        # lidar_names = []
        pattern = r'\[([A-Z_]+)\]lidar spin exceed'
                ...
                if result.returncode == 0:
                    ...
                    if output is not None:
                        if self.IPC_j6_logs_pose in output:
                            # print(output)
                            lidar_name = re.findall(pattern,output)
                            # print(f"{lidar_name} + {self.output_for_dirve_car} \n{output}")
                            return 1 , f"{lidar_name} + {self.output_for_dirve_car} \n{output}"
                        else:
...
if __name__ == "__main__":
    hostenv = host_env()
    hostenv.config_lidar_status()
    # hostenv.get_pose_status()
