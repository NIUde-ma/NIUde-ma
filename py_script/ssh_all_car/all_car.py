...
def deploy_to_car(car_id, ip_address, local_path="./check_qcraft", remote_path="~/ms"):
    """
    部署文件到指定车辆
    """
    try:
        ...
        if result.returncode == 0:
            # 设置文件权限
            chmod_command = f"ssh -o ConnectTimeout=5 -o StrictHostKeyChecking=no qcraft@{ip_address} 'chmod +x {remote_path}/check'"
        ...
        common.print_red(f"✗ {car_id} ({ip_address}) 部署超时")
        return {"car_id": car_id, "status": "error", "message": "部署超时"}
    except Exception as e:
        common.print_red(f"✗ {car_id} ({ip_address}) 部署异常: {str(e)}")
        return {"car_id": car_id, "status": "error", "message": str(e)}

def batch_deploy_all(local_path="./check_qcraft", remote_path="~/ms", max_workers=10):
    """
    批量部署到所有车辆
    """
    try:
...
def batch_deploy_selected(car_ids, local_path="./check_qcraft", remote_path="~/ms", max_workers=5):
    """
    批量部署到指定车辆
    """
    try:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--local', type=str, default="./check_qraft", help='本地文件路径 (默认: ./check_qraft)')
